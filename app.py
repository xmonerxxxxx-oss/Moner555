...
try:
    ...
    st.error("🔑 لم يتم العثور على GEMINI_API_KEY في الـ Secrets. أضفه من إعدادات التطبيق.")
    st.stop()
...
def classify_error(err_text: str) -> str:
    ...
    if "quota" in err_lower or "429" in err_text:
        return "⏳ تجاوزت الحد اليومي المجاني لـ Gemini. جرب بكرة أو انتظر شوية."
    ...
    if "deadline" in err_lower or "timeout" in err_lower or "503" in err_text:
        return "⌛ انتهت مهلة الاتصال بـ Gemini. جرب تاني."
...
7. إذا طُلب تنسيق: عرّف apply_styling(writer) بالشكل الصح:
   from openpyxl.styles import PatternFill, Font
   def apply_styling(writer):
       ...
       for row in ws.iter_rows(min_row=2, max_row=ws.max_row, max_col=ws.max_column):
           # مثال: if row[2].value and row[2].value > 5: row[0].fill = yellow
           pass  # استبدل بالشرط المطلوب
8. لا تستخدم print() أو open() أو pd.read_excel().
9. لو الملف مش موجود في dataframes استخدم أول ملف: df = list(dataframes.values())[0]
10. لا تكتب import في الكود — pd و np و io و PatternFill و Font كلها جاهزة مباشرة."""
