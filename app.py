...
def create_excel_bytes(df, apply_styling=None):
    ...
    out = io.BytesIO()
    # تنظيف DataFrame — استبدال NaN بـ None عشان Excel يعرضها صح
    df = df.where(pd.notna(df), other=None)
    with pd.ExcelWriter(out, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="Sheet1")
        ws = writer.sheets["Sheet1"]
        # تنسيق عرض الأعمدة تلقائياً بأمان
        for col_idx, col in enumerate(df.columns, 1):
            try:
                col_vals = df[col].dropna().astype(str).map(len)
                max_data = col_vals.max() if len(col_vals) > 0 else 10
                max_len = max(int(max_data), len(str(col))) + 4
...
12. *** ممنوع استخدام pd.concat لدمج DataFrames بأعمدة مختلفة *** — سيجعل كل القيم None.
    الصحيح: أعمل تقريرين منفصلين واجعل final_result هو الأهم، أو أضف عمود "النوع" لكل DataFrame قبل الدمج.
    مثال صحيح للدمج:
    df1["النوع"] = "موظف"
    df2["النوع"] = "صنف"
    final_result = pd.concat([df1[["النوع","الاسم","القيمة"]], df2[["النوع","الاسم","القيمة"]]], ignore_index=True)

## التلوين (مهم جداً — اتبع هذه القواعد بدقة):
12. عرّف apply_styling(writer) بهذا الشكل الوحيد المقبول:
def apply_styling(writer):
    ...
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row, max_col=ws.max_column):
        ...
        if val is not None:
            ...
            for cell in row:
                cell.fill = fill
13. لوّن كل خلايا الصف: for cell in row: cell.fill = fill
14. COLUMN_INDEX يبدأ من 0 حسب ترتيب أعمدة final_result.
15. *** ممنوع منعاً باتاً *** استخدام أي متغير معرّف خارج apply_styling بداخلها.
    مثال خاطئ: for row in ws.iter_rows(..., max_row=len(df_موظفين)+1)  ← خطأ
    مثال صحيح: for row in ws.iter_rows(min_row=2, max_row=ws.max_row, max_col=ws.max_column)
16. كل المتغيرات اللي تحتاجها apply_styling لازم تُعرَّف جوّاها مباشرة.
17. لا تستخدم pd.ExcelWriter داخل الكود — النظام بيعمله تلقائياً.
18. *** مهم جداً *** عند المقارنة في apply_styling لازم تتحقق من نوع القيمة أولاً:
    مثال خاطئ:  if val > 0.8  ← يكسر لو val كانت None أو str
    مثال صحيح:  if isinstance(val, (int, float)) and val > 0.8
    دايماً استخدم isinstance قبل أي مقارنة رقمية داخل apply_styling."""
